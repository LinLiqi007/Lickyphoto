...
export async function onRequestGet(context) {
  try {
    const raw = await context.env.DATA_KV.get('password');
    const data = raw !== null ? raw : JSON.stringify(DEFAULT_PASSWORD);
    return new Response(data, { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'no-store' } });
  } catch (e) {
    return new Response(JSON.stringify(DEFAULT_PASSWORD), { headers: { 'Content-Type': 'application/json' } });
  }
}
export async function onRequestPost(context) {
  try {
    ...
    await context.env.DATA_KV.put('password', body);
