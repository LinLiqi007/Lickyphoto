...
export async function onRequestGet(context) {
  try {
    const raw = await context.env.DATA_KV.get('messages');
    const data = raw !== null ? raw : JSON.stringify(DEFAULT_MESSAGES);
    return new Response(data, { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'no-store' } });
  } catch (e) {
    return new Response(JSON.stringify(DEFAULT_MESSAGES), { headers: { 'Content-Type': 'application/json' } });
  }
}
export async function onRequestPost(context) {
  try {
    ...
    await context.env.DATA_KV.put('messages', body);
